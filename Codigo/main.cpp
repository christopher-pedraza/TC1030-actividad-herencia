...
void calificarSerie() {
    ...
    if (id <= series.size() && id > 0) {
        ...
        std::cin >> capitulo;
        std::cout << "Inserta tu calificación para: " << series[id-1].getEpisodio(capitulo-1).getTitulo() << "(de 1 a 5)" << std::endl;
        std::cin >> calificacion;
        Episodio episodioTemporal = series[id-1].getEpisodio(capitulo-1);
        episodioTemporal.setCalificacion(calificacion);
        series[id-1].setEpisodio(episodioTemporal,capitulo-1);
    } else {
        std::cout << "ID inválido" << std::endl;
    }
}
